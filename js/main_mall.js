var hasclass = true;
			...
			var pPid = ["-1", "-1", "-1", "-1"],
				...
				pVid = ["", "", "", ""],
				pName = [ "铝杆","铝锭", "电解铜","铜杆" ];
			var dataDivId = ["cc_aluminum_rod", "cc_aluminum_ingot", "cc_electrolytic_copper", "cc_copper_rod"];
			var preCopperPrice = {
					"priceName": "沪铜",
					"newPrice": "",
					"sellPrice": "",
...
//				var wd = plus.nativeUI.showWaiting();
				jQuery.ajax({
					...
					url: C_URL + C_M_GOODS_LIST_CURRENT,
					data: param,
					dataType: "json",
					timeout: C_TIMEOUT,
...
//						wd.close();
						...
						if(response != null && response.page_arr != null && response.page_arr.pageNumber != null)
							lastPage[tabIdx] = response.page_arr.pageNumber;
						...
						isInitPage = isInitPage == null ? true : isInitPage;
						if(isInitPage) table.innerHTML = '';
						if(response.data_list != null && response.data_list.length > 0) {
							BindDataToPage(response.data_list, table);
...
//						wd.close();
						/*for(p in response) {
							console.info("[" + LS_P_MAIN_MALL + " ShowData] response." + p + " is:" + response[p]);
						}
						for(p in response["8"]) {
							console.info("[" + LS_P_MAIN_MALL + " ShowData] response[\"8\"]." + p + " is:" + response["8"][p]);
						}*/
						cPages[tabIdx] = cPages[tabIdx] + 1;
...
//						ShowToast(MSG_DATA_ERROR);
			...
			function bingData(response, targetID, isInitCurrentPage) {
				...
				var selObj = document.getElementById("a_sel_type"); 
			...
			function BindDataToPage(dataList, table,filter) {
				...
				mui.each(dataList, function(index, item) {
					...
					templateHtml += '</span>';
					templateHtml += '</div>';
					}else if(dataList[index].status=="SOLT_OUT"){
						templateHtml += '</span><span class="btn-product">';
					...
					templateHtml += '</span>';
					templateHtml += '</div>';
					}else if(dataList[index].status=="DOWN_SHELF"){
						templateHtml += '</span><span class="btn-product">';
