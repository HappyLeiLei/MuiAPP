(function($, window) {
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '<span class="mui-icon mui-icon-left-nav mui-pull-left mui-preview-header-close"></span><span class="mui-preview-indicator mui-title"></span><span class="mui-pull-right mui-icon mui-icon-trash mui-preview-header-delete"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.init = function() {
		...
		this.headerClose = this.element.querySelector($.classSelector('.preview-header-close'));
		this.headerDel = this.element.querySelector($.classSelector('.preview-header-delete'));
	...
	proto.initEvent = function() {
		...
		if(this.headerClose) {
			this.headerClose.addEventListener('tap', function(e) {
				self.close();
			});
		}
		if(this.headerDel) {
			this.headerDel.addEventListener('tap', function(e) {
				var btnArray = [{
					title: "删除",
					style: "destructive"
				}];
				plus.nativeUI.actionSheet({
					...
					switch(index) {
						...
						case 1:
							try{
								var groupName = self.cacheImgs[self.lastIndex].getAttribute("data-preview-group") || "1";
								var div = self.cacheImgs[self.lastIndex].parentNode;
								div.parentNode.removeChild(div);
								var addBtnElem = document.getElementById("btn_addImg_" + groupName);
								if(addBtnElem && addBtnElem.style.display === "none") {
									addBtnElem.style.display = "inline-block";
								}
								//刷新预览图
								var imgs = [];
								imgs = document.querySelectorAll("img[data-preview-src][data-preview-group='" + groupName + "']");
								if(imgs.length) {
									if(self.lastIndex >= imgs.length) self.lastIndex = imgs.length;
									self.addImages(groupName);
									self.openByGroup(self.lastIndex, groupName);
								}else{
									self.close();
								}
							}catch(e){
								plus.nativeUI.toast("图片删除异常，请稍后重试。", {"verticalAlign": "center"});
								//console.info("[mui.previewimage.withheader delete] exception:"+e);
							}
							break;
					}
				});
	...
	proto._initImgData = function(itemData, imgEl) {
		if(!itemData.sWidth) {
			...
			if(itemData.sScale > 1) itemData.sScale = 0.977;
		}
